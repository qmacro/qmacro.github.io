...
This produces the names of the genres of books that begin with the definite
article ("The ..."):
...
But it is _not_ included in the result set for the scoped query:
...
While both queries are materialised into `SELECT`s on
...
See the section [Combining the scoped syntax with infix
filters](#combining-the-scoped-syntax-with-infix-filters) later on in these
...
filters](https://cap.cloud.sap/docs/cds/cxl#infix-filters)

![infix filter diagram](/images/2026/05/infix-filter-diagram.png)
...
"restrictive" filter[<sup>2</sup>](#footnotes), and that works too:
...
If we wanted to annotate the element `e` with `@readonly`[<sup>3</sup>](#footnotes), we might write:
...
To disambiguate where the "join" is, we use a colon[<sup>4</sup>](#footnotes):
...
At [52:58](https://www.youtube.com/watch?v=OPOH3agULQ0&t=3178s) Patrice gives a
...
> Yes, it's not a coincidence that this is the same list that we considered [at
> the start of this episode](#introduction).
...
`CatalogService` (see the [Materialisation in DDL](#materialisation-in-ddl)
section):
...
The infix filter in the scoped query we saw earlier (see the [Combining the
scoped syntax with infix
filters](#combining-the-scoped-syntax-with-infix-filters) section):
...
Rounding off this episode at around
[57:13](https://www.youtube.com/watch?v=OPOH3agULQ0&t=3433s) Patrice shows how
expressive infix filters can be, with:
...
- the genre condition becomes an additional correlated subquery which itself
  then becomes part of the `EXISTS` conditions too
...
1. Later on in this episode at
   [54:48](https://www.youtube.com/watch?v=OPOH3agULQ0&t=3288s) I ask whether
